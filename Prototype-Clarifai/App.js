...
export default function App() {
  ...
  const clarifaiDetectObjectsAsync = async (base64Image) => {
    try {
      const newPredictions = await clarifaiApp.models.predict(
        Clarifai.FOOD_MODEL,
        { base64: base64Image },
        { maxConcepts: 3, minValue: 0.4 }
      );
      console.log(newPredictions);

      const clarifaiLabels = newPredictions.outputs[0].data.concepts.map(
        (concept) => ({
  ...
  const analyzePhoto = async () => {
    try {
      ...
      await clarifaiDetectObjectsAsync(base64ImageData);
    } catch (error) {
    ...
    <View style={styles.containerr}>
      {isCameraVisible ? (
        ...
        <View style={styles.mainContainer}>
          <Text style={styles.title}>Clarifai - Prototype</Text>
